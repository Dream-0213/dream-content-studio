...
import { todeskJulyContents } from "../todesk-july-content";
...
export default async function CasePage({params}: {params:Promise<{slug:string}>}) {
  ...
  return <PageShell>
    ...
    {slug === "feisuan-javaai" && <section className="section case-deliveries">
      <div className="section-heading"><div><h2>公开内容名单</h2></div><p>项目整体由 350 位博主参与，总阅读量达 200W+。当前已整理 200 篇公开内容，收录 600 个多平台发布链接。</p></div>
      <div className="delivery-proof" aria-label="飞算 JavaAI 推广成果数据"><div><strong>200W+</strong><span>项目总阅读量</span></div><div><strong>350</strong><span>位技术博主参与</span></div><div><strong>200</strong><span>篇公开内容</span></div><div><strong>600</strong><span>个多平台发布链接</span></div></div>
    ...
    {slug === "todesk-distribution" && <section className="section case-deliveries todesk-july-deliveries">
      <div className="section-heading"><div><h2>7 月公开<br/>内容成果</h2></div><p>围绕远程办公、跨设备协同、连接体验、安全与企业运维等真实需求，连续五周组织创作者完成产品测评与多平台分发。以下为 ToDesk 长期内容合作中可公开检索的部分发布记录。</p></div>
      <div className="delivery-proof" aria-label="ToDesk 7 月公开内容成果"><div><strong>50</strong><span>篇公开内容</span></div><div><strong>5周</strong><span>连续内容发布</span></div><div><strong>150</strong><span>个多平台链接</span></div><div><strong>多场景</strong><span>覆盖用户决策链路</span></div></div>
      <div className="delivery-list todesk-july-list">
        {todeskJulyContents.map((content,index)=><article className="delivery-item" key={`${content.name}-${content.links[0].url}`}>
          <span className="delivery-index">{String(index+1).padStart(2,"0")}</span>
      ...
      <p className="creator-note">当前展示长期合作中的 50 篇公开样本，不包含内部内容稿；外部平台内容可能因平台调整而发生变化。</p>
    </section>}
